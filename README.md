...
注意：ハイメモリに収まる範囲であれば、複数のISDファイルを指定可能です。すべてを最初にロードした後に順次再生されます。カーソルキーで制御も可能です。
...
- カーソル左 ... 前の動画に戻る (複数のISDを指定した場合)
- カーソル右 ... 次の動画に進む (複数のISDを指定した場合)
- XF1 ... 情報表示
...
* 0.4.0 (2023/12/26) ... ハイメモリに収まる限りの複数のISDファイルを指定できるようにした。
